...
def identity_ok(expected: str, actual: str) -> bool:
    e = set(norm(expected).split()); a = set(norm(actual).split())
    if not e or not a: return False
    strong = {x for x in e if any(ch.isdigit() for ch in x) or len(x) >= 6}
    if strong and len(strong & a) < max(1, len(strong) // 2): return False
    return len(e & a) / max(1, len(e)) >= 0.45


def identities(product: dict) -> list[str]:
    vals = [str(product.get('name') or '')] + [str(x) for x in product.get('identity_aliases') or []]
    return [x for i, x in enumerate(vals) if x and x not in vals[:i]]


def identity_any(product: dict, actual: str) -> bool:
    return any(identity_ok(x, actual) for x in identities(product))
...
def numeric_price(v):
    try:
        ...
        if 100 <= n <= 100000: return int(round(n))
    except Exception: pass
    return None


def product_offer_from_ld(blobs: list, product: dict):
    candidates=[]
    for blob in blobs:
        for d in walk(blob):
            ...
            if not identity_any(product,name): continue
            offers=d.get('offers'); offer_objs=offers if isinstance(offers,list) else [offers] if isinstance(offers,dict) else []
            for o in offer_objs:
                price=numeric_price(o.get('price')) or numeric_price(o.get('lowPrice'))
                if not price: continue
                candidates.append((price,name,str(o.get('availability') or ''),o.get('offerCount'),'JSON_LD_PRODUCT'))
    return sorted(candidates,key=lambda x:x[0])


async def verify_one(context, product: dict, sem: asyncio.Semaphore) -> dict:
    async with sem:
        page=await context.new_page(); url=str(product['url'])
        result={'sku':product['sku'],'name':product['name'],'url':url,'verified':False,'price':None,'availability':None,'verified_at':datetime.now(timezone.utc).isoformat(),'method':None,'host':urlparse(url).netloc}
        try:
            response=await page.goto(url,wait_until='domcontentloaded',timeout=45000)
            result['http_status']=int(response.status) if response else None
            title=await page.title(); h1=''
            try: h1=(await page.locator('h1').first.inner_text(timeout=3000)).strip()
            except Exception: pass
            result['rendered_title']=title; result['rendered_h1']=h1
            if not identity_any(product,' '.join([title,h1])):
                result['error']='PRODUCT_IDENTITY_MISMATCH'; return result

            scripts=await page.locator('script[type="application/ld+json"]').all_text_contents(); blobs=[]
            for text in scripts:
                try: blobs.append(json.loads(text))
                except Exception: continue
            offers=product_offer_from_ld(blobs,product)
            if offers:
            ...
            for sel in ('meta[property="product:price:amount"]','meta[itemprop="price"]','[itemprop="price"]','[data-price]'):
                ...
                try:
                    ...
                    if price:
                        ...
                        available=bool(re.search(r'\b(?:på lager|til butik|in stock|tilgængelig)\b',body,re.I))
            ...
            for m in PRICE_RE.finditer(body):
                ...
                if 100<=n<=100000: matches.append(n)
            if matches:
                result.update({'verified':True,'price':min(matches),'availability':'PAGE_PRICE','method':'PAGE_TEXT_PRICE'}); return result
            result['error']='NO_PRICE_OBJECT'; return result
        except Exception as exc:
            result['error']=f'{type(exc).__name__}: {str(exc)[:240]}'; return result
        finally:
            await page.close()
...
async def main() -> None:
    ...
    for kind,rows in v19.RETAIL_CANDIDATES.items():
        for row in rows:
            x=dict(row); x['kind']=kind; products.append(x)
    async with async_playwright() as p:
        ...
        rows=await asyncio.gather(*(verify_one(context,x,sem) for x in products)); await context.close(); await browser.close()
