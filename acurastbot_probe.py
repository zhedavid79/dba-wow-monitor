...
def reward_diagnostics():
    ...
    for d in devices:
        ...
        if not any(w in nl for w in wanted):
            continue
        for c in d.get('configurations') or []:
            cid=c.get('id')
            st=by_cfg.get(int(cid)) if cid is not None else None
            rows.append({
                'device':label,
                'configuration_id':cid,
                'ram':c.get('ram'),'storage':c.get('storage'),'cpu':c.get('cpu'),'variant':c.get('variant'),
                'processorCount':None if not st else st.get('processorCount'),
                'chainId':None if not st else st.get('chainId'),
                'expectedRewardMin':None if not st else st.get('expectedRewardMin'),
                'expectedRewardAvg':None if not st else st.get('expectedRewardAvg'),
                'expectedRewardMedian':None if not st else st.get('expectedRewardMedian'),
                'expectedRewardMax':None if not st else st.get('expectedRewardMax'),
                'calculatedAt':None if not st else st.get('calculatedAt'),
            })
    return {'stats_rows':len(stats),'chain_id_counts':dict(chain_ids),'selected_configurations':rows}
