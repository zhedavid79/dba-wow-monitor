...
async def resolve(context, href: str, seller_hint: str | None = None) -> dict:
    """Resolve a comparison redirect to a possible external route.

    Resolution is discovery only. `external_resolved` is intentionally false
    here and can only become true after the external retailer page itself has
    passed strict V22 authority verification.
    """
