...
async def inspect(context,p,sem,kmap):
    async with sem:
        page=await context.new_page();sku=str(p.get('sku') or '');name=str(p.get('name') or '');url=str(p.get('url') or '')
        out={'sku':sku,'kind':p.get('kind') or kmap.get(sku),'name':name,'product_url':url,'verified_at':datetime.now(timezone.utc).isoformat(),'identity_verified':False,'offers':[],'delivered_price_verified':False}
        try:
            ...
            for i in range(n):
                ...
                href=urljoin(url,href);txt=await evidence(a);vals=prices(txt)
                if not vals:continue
                displayed=min(vals);key=(href,displayed)
                if key in seen:continue
                seen.add(key);ship,se,sm=shipping(txt)
                raw.append({'displayed_price_dkk':displayed,'shipping_dkk':ship,'shipping_exact':se,'shipping_method':sm,'in_stock_card':bool(STOCK_RE.search(txt)),'promo_evidence':bool(PROMO_RE.search(txt)),'normal_price_dkk':normal_price(txt),'redirect_url':href,'evidence_text':txt[:650]})
            ...
            for row in raw[:12]:
                ...
                r.update({'item_price_dkk':item,'delivered_price_dkk':delivered,'delivered_price_method':method,'in_stock_verified':stock});r['buy_ready_offer']=bool(stock and r.get('concrete_store_offer') and total_exact and delivered is not None and r.get('buy_url'))
            ...
            for r in resolved:
                ...
                r['deal_type']='EXPLICIT_SALE' if r.get('explicit_discount_pct') else 'MARKET_DEAL' if dp and median and dp<=median*.90 else 'PROMOTION_EVIDENCE' if r.get('promo_evidence') else 'BEST_CURRENT_PRICE'
...
async def main():
    ...
    result={'generated_at':datetime.now(timezone.utc).isoformat(),'model':'V22_STORE_LEVEL_DELIVERED_PRICE_AND_DEALS','policy':'Purchase-ready new retail requires same-product identity, a store-specific Prisjagt offer_id/store_id, in-stock evidence and an authoritative delivery-inclusive total. If the external retailer redirect is Cloudflare-blocked in GitHub Actions, the verified Prisjagt Til butik redirect is retained as the purchase link and explicitly marked as not externally resolved.','products_total':len(rows),'delivered_price_verified':len(ready),'deal_candidates':len(deals),'external_urls_resolved':sum(1 for r in ready if r.get('external_url_resolved')),'rows':rows};OUT.write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
    for p in doc.get('products') or []:
        r=by.get(str(p.get('sku') or ''))
        if not r:continue
        p['retail_offer_v22']={k:r.get(k) for k in ('delivered_price_verified','delivered_price_dkk','item_price_dkk','shipping_dkk','delivered_price_method','buy_url','seller','store_id','offer_id','external_url_resolved','deal_type','normal_price_dkk','market_reference_delivered_dkk')}
    doc['offer_gate_v22']={'model':result['model'],'products_total':len(rows),'delivered_price_verified':len(ready),'deal_candidates':len(deals),'external_urls_resolved':result['external_urls_resolved'],'required_price_basis':'DELIVERED_PRICE_DKK'};RETAIL.write_text(json.dumps(doc,ensure_ascii=False,indent=2),encoding='utf-8')
