...
def _pulse_discovery_rows(bot_catalog):
    ...
    out=[]
    for p in pulse:
        full=str(p.get('pulse_model') or '').strip(); nf=norm(full)
        if not nf: continue
        hits=[(len(nb),nb,display) for nb,display in brand_names.items() if nf.startswith(nb+' ')]
        if not hits: continue
        _,nb,display=max(hits); parts=full.split(); model=' '.join(parts[len(nb.split()):]).strip()
...
def build_catalog(s):
    devices=getj(s,BOT_BASE+'/devices/with-counts'); catalog=[]
    for d in devices:
    ...
    for row in _pulse_discovery_rows(catalog):
        key=(norm(row['brand']),norm(row['model'])); old=by_key.get(key)
        if old is None:
            catalog.append(row); by_key[key]=row
        else:
            old['aliases'] |= row['aliases']; old['processor_count']=max(old['processor_count'],row['processor_count'])
...
def fallback_core_model(title,description,catalog):
    # Conservative escape hatch for real Core-compatible marketing models missing
    # from both discovery registries. It never makes a listing rank by itself:
    # same-listing verification, Core compatibility and unique Pulse reward match
    # still have to pass downstream.
    brand=detected_brand(title,catalog)
    if not brand or brand=='apple': return None
    cleaned=' '.join((title or '').split())
    cleaned=re.sub(r'\b(?:32|64|128|256|512|1024)\s*gb\b',' ',cleaned,flags=re.I)
    cleaned=re.sub(r'\b(?:3|4|6|8|10|12|16|18|24)\s*gb\s*ram\b',' ',cleaned,flags=re.I)
    cleaned=re.sub(r'\b(?:smartphone|mobiltelefon|mobil)\b',' ',cleaned,flags=re.I)
    cleaned=re.sub(r'\s+',' ',cleaned).strip(' -–—,|')
    if not cleaned: return None
    try:
        import acurast_valuation as v
        ok,_=v.core_compatibility(cleaned,title,description)
    except Exception:
        return None
    if not ok: return None
    model_brand=detected_brand(cleaned,catalog)
    if model_brand!=brand: return None
    return cleaned


def resolve_model(title,description,catalog):
    return model_of(title,catalog) or model_of(f'{title} {description}',catalog) or fallback_core_model(title,description,catalog)
...
def product_identity(title,description,model,price,catalog):
    ...
    model_brand=norm(hit['brand']) if hit else detected_brand(model,catalog)
    if explicit and model_brand and explicit!=model_brand:return False,f'explicit brand mismatch: {explicit} != {model_brand}'
    if explicit and not model_brand:return False,'resolved model brand not verified'
    ...
    complete=bool(COMPLETE_PHONE_RE.search(both)); functional=bool(FUNCTION_RE.search(both)); specs=bool(SPEC_RE.search(both)); explicit_model_title=bool(model_of(t,catalog) or fallback_core_model(t,d,catalog))
...
def add_search_docs(found,docs,source,catalog):
    for d in docs:
        ...
        r=found.setdefault(lid,{'listing_id':lid,'title_t0':title,'ask_t0':p,'model_t0':model_of(title,catalog) or fallback_core_model(title,'',catalog),'brand_t0':detected_brand(title,catalog),'sources':[]})
...
def main():
    ...
    out={'generated_at':datetime.now(timezone.utc).isoformat(),'gate_passed':True,'product_identity_gate':True,'max_ask_dkk':MAX_ASK_DKK,'data_gate':'AcurastBot registry + Acurast Pulse Mainnet model discovery + conservative Core-compatible title fallback + DBA same-listing verification','counts':{'category_pages':category_pages,'category_docs':category_docs,'catalog_models':len(catalog),'t0_unique':len(found),'t1_attempted':len(pool),'t1_verified_active_product':len(verified),'product_identity_excluded':stage_counts.get('PRODUCT_IDENTITY',0),'final_refetched':len(verified)},'ranked_by_verified_ask':verified,'excluded':[],'search_errors':[]}
