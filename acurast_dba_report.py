...
def detected_brand(text:str)->str|None:
    n=' '+norm(text)+' '
    aliases={'pixel':'google','galaxy':'samsung','redmi':'xiaomi'}
    hits=[]
...
def model_of(text:str,catalog)->str|None:
    nt=' '+norm(text)+' '; ct=compact(text); seller_brand=detected_brand(text); matches=[]
    ...
    for x in catalog:
        ...
        for a in x['aliases']:
            ...
            if boundary or compact_hit:
                ...
                specificity=sum(len(t) for t in model_tokens)
                matches.append((specificity,len(model_tokens),len(ca),x['label']))
...
def product_identity(title:str,description:str,model:str|None,price:int|None,catalog)->tuple[bool,str]:
    ...
    if detected_brand(t)=='apple': return False,'Apple/iPhone listing is not an Android Acurast Core candidate'
    ...
    if price is not None and price < 150 and not (complete and (functional or specs)): return False,'sub-150 DKK listing lacks strong complete-phone evidence'
    ...
    return True,'Mobiltelefoner category + <=1000 DKK + brand-aware supported-model + complete-phone identity passed'
...
def fetch_item(lid):
    ...
    for attempt in range(REQUEST_RETRIES):
        try:
            r=requests.get(ITEM_URL.format(id=lid),headers=HEADERS,timeout=TIMEOUT); r.raise_for_status(); payload=r.json(); item=payload.get('itemData') or {}; bound=canonical_id_from_payload(payload)
...
def add_search_docs(found:dict,docs:list,source:str,catalog):
    ...
    for d in docs:
        ...
        model=model_of(title,catalog); is_new=lid not in found
...
def t1_pool(found:dict)->list[dict]: return sorted(found.values(),key=lambda r:(int(r['ask_t0']),r['listing_id']))
...
def verify_t1(r,catalog):
    ...
    except Exception as e: return None,{**r,'reason':f'T1 fetch failed: {e!r}'}
...
def main():
    s=requests.Session(); Path('results').mkdir(exist_ok=True); catalog=build_catalog(s); found={}; search_errors=[]; category_pages=0; category_docs=0; previous_ids=None
    for page in range(1,CATEGORY_MAX_PAGES+1):
        try:
            payload=getj(s,SEARCH_API,{'product_category':MOBILE_CATEGORY,'sort':'PRICE_ASC','page':page}); docs=payload.get('docs') or []
            ...
            previous_ids=ids; category_pages+=1; category_docs+=len(docs); add_search_docs(found,docs,f'category:{page}',catalog)
            page_prices=[amount(d.get('price')) for d in docs]; page_prices=[p for p in page_prices if p is not None]
        ...
        time.sleep(.01)
    if category_pages==0:
        out={'generated_at':datetime.now(timezone.utc).isoformat(),'gate_passed':False,'reason':'PRICE DATA GATE FAILED — DBA Mobiltelefoner category unavailable after retries','search_errors':search_errors}; Path('results/acurast_latest.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8'); raise SystemExit(out['reason'])
    for q in SALVAGE_QUERIES:
        try:
            payload=getj(s,SEARCH_API,{'q':q,'sort':'PRICE_ASC'}); docs=payload.get('docs') or []
        ...
        time.sleep(.01)
    if not found:
        out={'generated_at':datetime.now(timezone.utc).isoformat(),'gate_passed':False,'reason':'PRICE DATA GATE FAILED — no structured DBA candidates <= 1000 DKK','search_errors':search_errors}; Path('results/acurast_latest.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8'); raise SystemExit(out['reason'])
